...
while(true)
    while(true)
% -------------------------------------------------------------------------
        % update no of observation
        n = length(code);
        % initialize all observation
        azmth=zeros(n,1); elev=zeros(n,1); dist=zeros(n,1); 
        A=zeros(n,4); computed_range=zeros(n,1); observed_range=zeros(n,1);
        Q=zeros(n,n);
...
% -------------------------------------------------------------------------
        ...
        if max(abs(residuals)) > limit
            [res_max, iprn_rej,index] = findmax_residuals(residuals, iprn_obs, iprn_rej);
            [iprn_obs,sat_pos,dtsat,code]=reject_obs(iprn_obs,sat_pos,dtsat,code,index);
        else
            break;
        end
